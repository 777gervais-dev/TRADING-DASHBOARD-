...
with tab3:
    ...
    if df_liq is not None and len(df_liq) > 10:
        ...
        if 'Volume' in df_plot_liq.columns:
            # Sécurisation des données : conversion en listes et gestion des NaN
            vols = df_plot_liq['Volume'].fillna(0).squeeze()
            closes = df_plot_liq['Close'].fillna(0).squeeze()
            opens = df_plot_liq['Open'].fillna(0).squeeze()
            
            # Conversion explicite en listes pour éviter tout conflit avec Pandas/Numpy
            v_list = vols.tolist() if hasattr(vols, 'tolist') else list(vols)
            c_list = closes.tolist() if hasattr(closes, 'tolist') else list(closes)
            o_list = opens.tolist() if hasattr(opens, 'tolist') else list(opens)
            
            # Calcul des couleurs avec une valeur par défaut sécurisée
            vol_colors = ['#00ff8866' if c >= o else '#ff2d5566' for c, o in zip(c_list, o_list)]
        ...
        fig_liq.update_xaxes(gridcolor='#1a3a5c', showgrid=True, rangeslider_visible=False)
        fig_liq.update_yaxes(gridcolor='#1a3a5c', showgrid=True)
