...
def compute_pivot_points(df):
    ...
    # Find last candle where H != L (non-flat/valid candle)
    row = None
    for i in range(2, min(10, len(df)) + 1):
        ...
        if h != l and h > 0 and l > 0 and c > 0:
            row = candidate
            break
    if row is None:
        row = df.iloc[-2]
...
with tab3:
    ...
    if df_liq is not None and len(df_liq) > 10:
        ...
        if 'Volume' in df_plot_liq.columns:
            ...
            closes_v = df_plot_liq['Close'].squeeze().values
            opens_v = df_plot_liq['Open'].squeeze().values
            # Use solid hex colors (no alpha in hex) — fully compatible
            vol_colors = ['#00cc66' if c >= o else '#cc2244'
                          for c, o in zip(closes_v, opens_v)]
            fig_liq.add_trace(go.Bar(
                x=df_plot_liq.index.astype(str).tolist(),
                y=vols.values.tolist(),
        ...
        if liq['vol_profile']:
            ...
            poc_val = liq['poc'] or 0
            vp_colors = ['#00d4ff' if abs(v['price'] - poc_val) == min(
                abs(vv['price'] - poc_val) for vv in vp) else '#1a6080' for v in vp]
            fig_liq.add_trace(go.Bar(
                x=[v['volume'] / max_vol for v in vp],
                y=[v['price'] for v in vp],
                orientation='h',
                marker=dict(color=vp_colors, opacity=0.8),
                name="Vol Profile",
                showlegend=False,
            ), row=1, col=2)
...
with tab4:
    ...
    if df_chart is not None and len(df_chart) > 20:
        ...
        # Bollinger Bands
        fig_chart.add_trace(go.Scatter(x=df_chart.index, y=bb_upper, line=dict(color='rgba(255,215,0,0.27)', width=1), name="BB Up", showlegend=False), row=1, col=1)
        fig_chart.add_trace(go.Scatter(x=df_chart.index, y=bb_lower, line=dict(color='rgba(255,215,0,0.27)', width=1), fill='tonexty', fillcolor='rgba(255,215,0,0.03)', name="BB Low", showlegend=False), row=1, col=1)
        fig_chart.add_trace(go.Scatter(x=df_chart.index, y=bb_mid, line=dict(color='rgba(255,215,0,0.40)', width=1, dash='dot'), name="BB Mid", showlegend=False), row=1, col=1)
        ...
        fig_chart.add_trace(go.Bar(x=df_chart.index, y=macd_hist.tolist(),
        ...
        fig_chart.add_hline(y=70, line_color='rgba(255,45,85,0.4)', line_dash='dot', row=3, col=1)
        fig_chart.add_hline(y=30, line_color='rgba(0,255,136,0.4)', line_dash='dot', row=3, col=1)
